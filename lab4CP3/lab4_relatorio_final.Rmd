...
python_quarta <- py %>% filter(week_day %in% c("quarta-feira"))
python_sexta <- py %>% filter(week_day %in% c("sexta-feira"))

mark <- git_data_filter %>% filter(file_extension %in% c("md"))
mark_quarta <- mark %>% filter(week_day %in% c("quarta-feira"))
mark_sexta <- mark %>% filter(week_day %in% c("sexta-feira"))
```
...
A Figura 8 e 9 mostra o intervalo de confiança encontrado para a amostra das de arquivos python modificados nas quartas e sextas, respectivamente.
...
```{r, fig.align='center', fig.cap="Figura 8 - Intervalo de confiança da diferença para python nas quartas-feiras"}
plot_sd_ic(p1)
```
...
```{r, warning=FALSE, fig.align='center', fig.cap="Figura 10 - Comparação entre os intervalos de confiança dos desvios padrões das amostras de python"}
...
```{r, fig.align='center', fig.cap="Figura 13 - Intervalo de confiança da diferença para python nas quartas-feiras"}
plot_sd_ic(m2)
```


```{r, warning=FALSE, fig.align='center', fig.cap="Figura 14 - Comparação entre os intervalos de confiança dos desvios padrões das amostras de python"}
...
```{r, fig.align='center', fig.cap="Figura 15 - Intervalo de confiança da diferença para python das modificações feitas na quarta-feira e na sexta-feira"}
# Agora a diferença das médias
...
```{r, fig.align='center', fig.cap="Figura 16 - Intervalo de confiança da diferença para python das modificações feitas na quarta-feira e na sexta-feira"}
