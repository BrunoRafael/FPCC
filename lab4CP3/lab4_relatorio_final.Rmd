...
git_languages_data1  <- git_data_filter %>%
  ...
  filter(week_day %in% c("segunda"))

git_languages_data2  <- git_data_filter %>% 
  filter(week_day %in% c("quarta", "sexta")) %>%
...
```{r, fig.align='center', fig.cap="Figura 4 - Amostra de um bootstrap para javascript"}
...
b1 = git_languages_data1 %>% filter(file_extension %in% c("java")) %>%
  bootstrap(median(users))
plot_ic(b1)
```
