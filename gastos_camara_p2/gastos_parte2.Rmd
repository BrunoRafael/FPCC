...
```{r, warning=FALSE}
gastos_titulares %>% group_by(ideologia_simplificada, txNomeParlamentar) %>% 
filter(numAno <= 2017 & numAno >= 2009) %>%
summarise(Total = count(ideologia_simplificada))
...
gastos_titulares %>% group_by(ideologia_simplificada, txNomeParlamentar) %>% 
filter(numAno <= 2017 & numAno >= 2009) %>% ggplot() + 
  geom_bar(mapping = aes(x = ideologia_simplificada), stat = "count") + 
  scale_y_continuous(formata_label)
  coord_flip()
```
...
geom_boxplot(alpha = 0.4)+
scale_y_log10(labels=formata_label)+
facet_grid(ideologia_simplificada ~ .)+
labs(x="Estado(UF)", y = "Total de gastos R$ (Em milhões)")+
coord_flip()
...
coord_flip()+
theme(axis.text.x=element_text(angle=-90,hjust=1,vjust=0.5))+
labs(x = "Ideologia", y = "Gastos (R$ em milhões)", fill="Ano")+ scale_fill_continuous(breaks=unique(gastos_titulares$numAno))
```